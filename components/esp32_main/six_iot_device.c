...
static esp_err_t s_http_response_init(six_http_response_ctx_t *ctx, size_t initial_size, size_t max_size) {
...
static esp_err_t s_http_response_append(six_http_response_ctx_t *ctx, const void *data, size_t data_len) {
	...
	if (ctx->length > ctx->max_size || data_len > ctx->max_size - ctx->length) {
		ESP_LOGE(TAG, "HTTP response exceeds limit: current=%u incoming=%u max=%u", (unsigned)ctx->length,
				 (unsigned)data_len, (unsigned)ctx->max_size);
		return ESP_ERR_NO_MEM;
	}
	...
	if (required > ctx->capacity) {
		...
		char *new_data = realloc(ctx->data, new_capacity + 1U);

		if (new_data == NULL) {
			ESP_LOGE(TAG, "Failed to grow HTTP response buffer from %u to %u", (unsigned)ctx->capacity,
...
static void s_report_provision_status(six_iot_event_t status, char *msg, void *args) {
	...
	if (status == PROVISION_STATUS_DEVICE_AUTH_EXG_TOKENS_SUCCEED) {
		...
		if (err != ESP_OK) {
			ESP_LOGW(TAG, "Failed to post ID-token event: %s", esp_err_to_name(err));
		}

		data = tokens->access_token;
		size = strlen(data) + 1U;

		err = esp_event_post_to(s_six_iot_loop, SIX_IOT_EVENT, PROVISION_STATUS_DEVICE_AUTH_EXG_ACCESS_TOKEN_SUCCEED,
								data, size, SIX_IOT_SDK_EVENT_TICK);

		if (err != ESP_OK) {
			ESP_LOGW(TAG, "Failed to post access-token event: %s", esp_err_to_name(err));
		}
	...
	esp_err_t err = esp_event_post_to(s_six_iot_loop, SIX_IOT_EVENT, status, args, size, SIX_IOT_SDK_EVENT_TICK);

	if (err != ESP_OK) {
		ESP_LOGW(TAG, "Failed to post provisioning event %d: %s", (int)status, esp_err_to_name(err));
	}
}
...
esp_err_t _http_rest_for_tokens_event_handler(esp_http_client_event_t *evt) {
	...
	six_http_response_ctx_t *ctx = (six_http_response_ctx_t *)evt->user_data;
	...
	case HTTP_EVENT_ON_HEADER:
		ESP_LOGD(TAG, "HTTP_EVENT_ON_HEADER, key=%s", evt->header_key != NULL ? evt->header_key : "<unknown>");
		break;
	...
	case HTTP_EVENT_ON_DATA:
		...
		esp_err_t err = s_http_response_append(ctx, evt->data, (size_t)evt->data_len);

		if (err != ESP_OK) {
			ESP_LOGE(TAG, "Unable to store token HTTP response");
	...
	case HTTP_EVENT_ON_FINISH:
		...
		ESP_LOGD(TAG, "HTTP_EVENT_ON_FINISH, response length=%u", (unsigned)ctx->length);
		...
		if (status_code < 200 || status_code >= 300) {
			ESP_LOGW(TAG, "Token endpoint returned HTTP status %d", status_code);
		...
		cJSON *resp_json = cJSON_ParseWithLengthOpts(ctx->data, ctx->length, NULL, 0);
		...
		bool success = (id_token != NULL && access_token != NULL);

		/*
		 * The callback borrows these pointers for the duration of the call.
		 * It must not retain them.
		 */
		ctx->token_handler(success, id_token, access_token);
...
static void s_http_rest_for_tokens(const char *token_endpoint, const char *jwt, six_iam_token_handler_t token_handler) {
	...
	esp_err_t err = s_http_response_init(&response, HTTP_INITIAL_BUFFER_SIZE, MAX_TOKEN_RESPONSE_SIZE);
	...
	int post_data_len = snprintf(NULL, 0, SIX_IAM_DEVICE_JWT_ASSERTION, jwt);
	...
	char *post_data = malloc((size_t)post_data_len + 1U);
	...
	int written = snprintf(post_data, (size_t)post_data_len + 1U, SIX_IAM_DEVICE_JWT_ASSERTION, jwt);
	...
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "Failed to set token HTTP method: %s", esp_err_to_name(err));
	...
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "Failed to set token Content-Type: %s", esp_err_to_name(err));

		goto cleanup;
	}

	err = esp_http_client_set_post_field(client, post_data, (size_t)post_data_len);

	if (err != ESP_OK) {
		ESP_LOGE(TAG, "Failed to set token POST field: %s", esp_err_to_name(err));

		goto cleanup;
	}

	ESP_LOGD(TAG, "Sending token request to IAM endpoint");
	...
	if (err != ESP_OK) {
		 ...
		 */
		if (response.length == 0U && response.token_handler != NULL) {
	...
	ESP_LOGD(TAG, "HTTP POST Status=%d content_length=%lld response_length=%u", esp_http_client_get_status_code(client),
			 esp_http_client_get_content_length(client), (unsigned)response.length);
...
cleanup:
	...
	if (err != ESP_OK) {
		ESP_LOGW(TAG, "HTTP client cleanup failed: %s", esp_err_to_name(err));
	}
}
...
void _six_iam_exchange_tokens(const char *token_endpoint, const char *jwt, six_iam_token_handler_t token_handler) {
	...
	s_http_rest_for_tokens(token_endpoint, jwt, token_handler);
}

void _six_iam_exchange_device_tokens_with_local_key(const char *private_key) {
	if (private_key == NULL || s_iot_cfg == NULL || s_iot_cfg->iam.device_guid == NULL) {
...
bool _six_is_tokens_valide_in_nvs(bool post_event) {
	...
	bool valid = id_token_in_nvs != NULL && !six_iam_token_expired(id_token_in_nvs, Second) &&
				 access_token_in_nvs != NULL && !six_iam_token_expired(access_token_in_nvs, Second);
	...
	if (post_event && s_six_iot_loop != NULL) {
		...
		if (err != ESP_OK) {
			ESP_LOGW(TAG, "Failed to post cached ID token event: %s", esp_err_to_name(err));
		}
		...
		if (err != ESP_OK) {
			ESP_LOGW(TAG, "Failed to post cached access token event: %s", esp_err_to_name(err));
		}
	}
...
void six_iam_exchange_device_tokens(six_iot_config_t *iot_config, esp_event_loop_handle_t loop_handle) {
	...
	s_report_provision_status(PROVISION_STATUS_DEVICE_AUTH_OBTAIN_LOCAL_KEY, "Obtain the device RSA key from flash",
							  NULL);

	if (private_key != NULL) {
		s_report_provision_status(PROVISION_STATUS_DEVICE_AUTH_OBTAIN_LOCAL_KEY_SUCCEED,
								  "Obtain the device RSA key from flash succeed!", NULL);
		...
		s_report_provision_status(PROVISION_STATUS_DEVICE_AUTH_OBTAIN_LOCAL_KEY_FAIL,
								  "Can't obtain the device RSA key from flash!", NULL);
